...
import { FiSearch, FiEdit2, FiTrash2, FiUserX, FiCheckCircle, FiKey } from 'react-icons/fi';
    ...
    >
      ...
      {resetMsg && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm font-semibold"
        >
          ✅ {resetMsg}
        </motion.div>
      )}
          ...
          <table className="w-full text-left border-collapse min-w-[900px] font-poppins">
            <thead>
              <tr className="text-gray-400 text-xs uppercase tracking-wider">
                ...
                <th className="py-5 px-6 font-semibold">Password</th>
              ...
              {filteredUsers.length > 0 ? filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-white/[0.04] transition-colors rounded-2xl group/row">
                  ...
                  <td className="py-4 px-6">
                    ...
                    <span className="font-mono text-gray-500 tracking-widest text-sm select-none">••••••••</span>
                  </td>
                  <td className="py-4 px-6">
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="p-2 rounded-xl bg-white/[0.02] border border-white/[0.05] hover:border-violet-500/50 hover:bg-violet-500/10 text-gray-400 hover:text-violet-400 transition-all shadow-lg"
                      ...
                      >
                        {user.status === 'Active' ? <FiUserX size={15} /> : <FiCheckCircle size={15} />}
                      </button>
                      <button
