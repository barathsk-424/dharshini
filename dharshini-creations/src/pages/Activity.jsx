...
const allActivities = [
  ...
  { id: 5, type: 'order', title: 'Order Placed', desc: 'You placed a new order DC-003.', date: 'May 18, 9:00 AM', icon: FiCheckCircle, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
  { id: 6, type: 'wishlist', title: 'Removed from Wishlist', desc: 'You removed "Classic Hoodie" from your wishlist.', date: 'May 15, 3:45 PM', icon: FiHeart, color: 'text-gray-400', bg: 'bg-gray-500/10' },
  { id: 7, type: 'login', title: 'Logged In', desc: 'Logged into your account from a new device.', date: 'May 10, 8:15 AM', icon: FiLogIn, color: 'text-blue-400', bg: 'bg-blue-500/10' },
];

export default function Activity() {
  const [visibleCount, setVisibleCount] = useState(4);
  const handleLoadMore = () => setVisibleCount(p => p + 4);
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <motion.div className="glass-card p-6 md:p-10 relative overflow-hidden" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }}>
          ...
          <div className="relative border-l border-white/10 ml-4 md:ml-6 space-y-8 pb-4">
            {allActivities.slice(0, visibleCount).map((item, i) => {
          ...
          {visibleCount < allActivities.length && (
            <div className="pt-8 text-center border-t border-white/5 mt-4">
              <button onClick={handleLoadMore} className="text-sm font-semibold text-purple-400 hover:text-white transition-colors interactive">
                Load More Activity
              </button>
            </div>
          )}
