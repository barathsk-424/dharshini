      ...
      <div className="section-container max-w-3xl mx-auto" style={{ paddingTop: 40 }}>
        {submitted ? (
          <motion.div className="glass-card p-12 text-center" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
            ...
            <p className="text-sm" style={{ color: '#B8B8B8' }}>We'll review your design and send a quote via WhatsApp within 24 hours.</p>
          ...
          <div className="glass-card p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                <label className="text-xs font-semibold block mb-2" style={{ color: '#B8B8B8' }}>Phone / WhatsApp</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                <label className="text-xs font-semibold block mb-2" style={{ color: '#B8B8B8' }}>Shirt Type</label>
                <select value={form.shirtType} onChange={e => update('shirtType', e.target.value)} className="w-full px-4 py-3 rounded-xl text-sm border outline-none" style={{ borderColor: 'rgba(106,13,173,0.3)', color: '#B8B8B8', background: 'rgba(20,10,40,0.8)' }}>
                  <option value="tshirt" style={{ background: '#0a0a0a' }}>T-Shirt</option>
                  <option value="shirt" style={{ background: '#0a0a0a' }}>Formal Shirt</option>
                </select>
              </div>
              <div>
                <label className="text-xs font-semibold block mb-2" style={{ color: '#B8B8B8' }}>Design Position</label>
                <select value={form.position} onChange={e => update('position', e.target.value)} className="w-full px-4 py-3 rounded-xl text-sm border outline-none" style={{ borderColor: 'rgba(106,13,173,0.3)', color: '#B8B8B8', background: 'rgba(20,10,40,0.8)' }}>
                  {['chest', 'sleeve-left', 'sleeve-right', 'back'].map(p => <option key={p} value={p} style={{ background: '#0a0a0a' }}>{p.replace('-', ' ')}</option>)}
                </select>
              </div>
            </div>
            <div className="mb-6">
              <label className="text-xs font-semibold block mb-2" style={{ color: '#B8B8B8' }}>Shirt Color</label>
              <div className="flex gap-3 flex-wrap">
                {shirtColors.map(c => (
                  <button key={c.name} onClick={() => update('color', c.name)} className="flex items-center gap-2 px-3 py-2 rounded-lg interactive"
                    ...
                    <span className="text-xs" style={{ color: '#B8B8B8' }}>{c.name}</span>
                  </button>
                ))}
              </div>
            </div>
            <div className="mb-6">
              <label className="text-xs font-semibold block mb-2" style={{ color: '#B8B8B8' }}>Reference Images</label>
              <div className="border-2 border-dashed rounded-xl p-6 text-center interactive" style={{ borderColor: '#6A0DAD' }}
                ...
                <p className="text-sm mb-2" style={{ color: '#B8B8B8' }}>Drag & drop or <label className="text-purple-400 cursor-pointer interactive">browse<input type="file" multiple accept="image/*" className="hidden" onChange={e => setFiles(p => [...p, ...Array.from(e.target.files)])} /></label></p>
                {files.length > 0 && <p className="text-xs mt-2" style={{ color: '#4ade80' }}>✓ {files.length} file(s) selected</p>}
              </div>
            </div>
            <div className="mb-6">
              <label className="text-xs font-semibold block mb-2" style={{ color: '#B8B8B8' }}>Measurements</label>
            ...
            <div className="mb-8">
              <label className="text-xs font-semibold block mb-2" style={{ color: '#B8B8B8' }}>Special Instructions</label>
