...
// Unique accent color per product based on category
const productAccent = {
  1: '#FB923C',  // Fabric Painting - orange
  2: '#38BDF8',  // Embroidery - sky blue
  3: '#818CF8',  // Combo - indigo
};
      ...
      <div className="section-container">
        ...
        >
          {['All', 'Trending', 'New Arrivals', 'Festival', 'Couple', 'Anime', 'Floral'].map(tag => {
            const key = tag.toLowerCase().replace(' ', '-').replace('arrivals', 'arrival');
            const tc = tagColors[key];
            return (
              <button
                key={tag}
                className="tag-badge interactive hover:scale-105 transition-transform"
                style={tc ? { background: tc.bg, color: tc.text, border: `1px solid ${tc.border}` } : {}}
              >
                {tag}
              </button>
            );
        ...
        <div className="overflow-x-auto pb-4 -mx-6 px-6 scrollbar-hide" style={{ scrollbarWidth: 'none' }}>
          <div className="flex gap-6" style={{ width: 'max-content' }}>
            {products.map((product, i) => {
              const accent = productAccent[product.categoryId] || '#B266FF';
              return (
                <motion.div
                  key={product.id}
