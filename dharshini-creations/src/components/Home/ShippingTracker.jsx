...
const steps = [
  { key: 'packed', label: 'Order Packed', icon: FiPackage, emoji: '📦' },
  { key: 'shipped', label: 'Shipped', icon: FiTruck, emoji: '🚚' },
  { key: 'out', label: 'Out for Delivery', icon: FiTruck, emoji: '🏍️' },
  { key: 'delivered', label: 'Delivered', icon: FiCheckCircle, emoji: '✅' },
];
    ...
    <section ref={ref} className="relative">
      <div className="section-container">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}>
          <p className="section-subtitle">Where's My Order?</p>
        ...
        <motion.div className="max-w-3xl mx-auto mt-8" initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.2 }}>
          <div className="glass-card p-8">
            <div className="flex gap-3 mb-8">
              ...
              <button onClick={handleTrack} className="btn-primary px-6 interactive"><FiSearch size={16} /></button>
            </div>
            ...
            {tracking && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <div className="relative mb-8 p-6 rounded-2xl" style={{ background: 'rgba(138,43,226,0.05)' }}>
                  <svg viewBox="0 0 400 300" className="w-full max-h-48">
                    ...
                    {/* Source: Tamil Nadu */}
                    <circle cx="200" cy="250" r="6" fill="#8A2BE2">
                      <animate attributeName="r" values="4;8;4" dur="2s" repeatCount="indefinite" />
                    </circle>
                    <text x="210" y="255" fill="#B8B8B8" fontSize="9" fontFamily="Poppins">Tamil Nadu</text>
                    {/* Destination */}
                    <circle cx="200" cy={tracking.dest === 'Mumbai' ? 150 : tracking.dest === 'Delhi' ? 80 : 200} r="5" fill="#B266FF" />
                    <text x="210" y={tracking.dest === 'Mumbai' ? 155 : tracking.dest === 'Delhi' ? 85 : 205} fill="#B266FF" fontSize="9" fontFamily="Poppins">{tracking.dest}</text>
                    {/* Animated line */}
                ...
                <div className="flex items-center justify-between relative">
                  <div className="absolute top-5 left-0 right-0 h-0.5" style={{ background: 'rgba(106,13,173,0.3)' }} />
                  <motion.div className="absolute top-5 left-0 h-0.5" style={{ background: 'linear-gradient(90deg, #8A2BE2, #B266FF)', width: `${((statusIndex + 1) / steps.length) * 100}%` }}
                  ...
                  {steps.map((step, i) => (
                    <div key={step.key} className="relative z-10 flex flex-col items-center gap-2">
                      ...
                      <span className="text-[10px] font-poppins text-center max-w-[70px]" style={{ color: i <= statusIndex ? '#B266FF' : '#666' }}>{step.label}</span>
                    </div>
                  ))}
                </div>

                <div className="mt-6 text-center">
                  <p className="text-sm" style={{ color: '#B8B8B8' }}>Destination: <span style={{ color: '#B266FF' }}>{tracking.dest}</span></p>
