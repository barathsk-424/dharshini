    ...
    <section ref={ref} className="relative">
      <div className="section-container">
        ...
        <motion.div className="max-w-3xl mx-auto mt-8 glass-card p-8" initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.2 }}>
          <div className="mb-8">
            <h4 className="font-cinzel text-sm font-semibold tracking-widest mb-4" style={{ color: '#B8B8B8' }}>SHIRT TYPE</h4>
            <div className="flex gap-4">
              {[{ value: 'tshirt', label: 'T-Shirt', price: '₹199', emoji: '👕' }, { value: 'shirt', label: 'Formal Shirt', price: '₹399', emoji: '👔' }].map(type => (
                <button key={type.value} onClick={() => setShirtType(type.value)} className="flex-1 p-5 rounded-xl text-center interactive transition-all"
                  style={{ background: shirtType === type.value ? 'rgba(138,43,226,0.2)' : 'rgba(138,43,226,0.05)', border: `2px solid ${shirtType === type.value ? '#B266FF' : 'rgba(106,13,173,0.2)'}` }}>
                  <div className="text-3xl mb-2">{type.emoji}</div>
                  <p className="font-poppins font-semibold text-sm" style={{ color: shirtType === type.value ? '#F5F5F5' : '#B8B8B8' }}>{type.label}</p>
                  <p className="text-xs mt-1" style={{ color: '#B266FF' }}>Base: {type.price}</p>
                </button>
              ))}
            </div>
          </div>
          <div className="mb-8">
            <h4 className="font-cinzel text-sm font-semibold tracking-widest mb-4" style={{ color: '#B8B8B8' }}>ADD-ONS</h4>
              ...
              {Object.entries(addonLabels).map(([key, { label, emoji }]) => {
                ...
                return (
                  <label key={key} className="flex items-center gap-3 p-4 rounded-xl interactive cursor-pointer transition-all"
                    style={{ background: addons[key] ? 'rgba(138,43,226,0.15)' : 'rgba(138,43,226,0.05)', border: `1px solid ${addons[key] ? '#B266FF' : 'rgba(106,13,173,0.15)'}` }}>
                    <input type="checkbox" checked={addons[key]} onChange={() => setAddons(p => ({ ...p, [key]: !p[key] }))} className="w-4 h-4 accent-purple-500" />
                    <span className="text-lg">{emoji}</span>
                    <span className="flex-1 text-sm font-poppins" style={{ color: '#F5F5F5' }}>{label}</span>
                    <span className="font-cinzel font-bold text-sm" style={{ color: '#B266FF' }}>+₹{rule?.price || 0}</span>
                  </label>
                );
              })}
            </div>
          ...
          <div className="mb-8">
            <h4 className="font-cinzel text-sm font-semibold tracking-widest mb-4" style={{ color: '#B8B8B8' }}>SHIPPING</h4>
            <div className="flex gap-3">
              <input type="text" value={pincode} onChange={e => setPincode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="Enter pincode"
                className="flex-1 px-4 py-3 rounded-xl text-sm bg-white/5 border outline-none focus:border-purple-500 transition-colors" style={{ borderColor: 'rgba(106,13,173,0.3)', color: '#F5F5F5' }} />
              <button onClick={calculateShipping} className="btn-primary text-sm py-3 px-6 interactive">Calculate</button>
            </div>
            {shipping && (
          ...
          <div className="p-6 rounded-2xl text-center" style={{ background: 'linear-gradient(135deg, rgba(138,43,226,0.15), rgba(178,102,255,0.08))' }}>
            <p className="text-xs font-poppins tracking-widest mb-2" style={{ color: '#B8B8B8' }}>ESTIMATED TOTAL</p>
            <div className="price-display">₹<AnimatedNumber value={total + (shipping?.cost || 0)} /></div>
          </div>
        </motion.div>
      </div>
    </section>
